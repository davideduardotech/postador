...
function openInstagram() {
    const popup = document.getElementById("select-instagram")
    popup.classList.remove("hidden")
}
...
function openPreview() {
    ...
    if (media.length > 0) {
        ...
        imgPreviewSrc.src = window.URL.createObjectURL(media[0])
    } else {
...
function savePost() {
    ...
    if (media.length > 0) {
        ...
        fetch('/app/posts/upload-image', {
            ...
            .then((filename) => {
                fetch('/app/save-post', {
                    ...
                    body: JSON.stringify({ content: content.value, pages: pages, groups: groups, accountsInstagram: accountsInstagram, schedule: schedule.checked, day: day.value, hour: hour.value, interval: interval.value, media: filename, type: "media" })
                })
                    .then(async (response) => {
                        if (response.status == 200) {
                            ...
                            mediaSelectedArea.innerHTML = ""
                            media.length = 0
                            selectMediaArea.classList.remove("hidden")
                            linkSelectedArea.classList.add("hidden")
                            mediaSelectedArea.classList.add("hidden")

                            alertMessage("success", "Processo concluído")
        ...
        return fetch('/app/save-post', {
            method: 'POST',
            headers: {
                "Content-Type": "application/json",
            },
            body: JSON.stringify({ content: content.value, link: link, pages: pages, groups: groups, accountsInstagram: accountsInstagram, schedule: schedule.checked, day: day.value, hour: hour.value, interval: interval.value, type: "text-link" })
        })
            .then(async (response) => {
                if (response.status == 200) {
                    typePost = ""
