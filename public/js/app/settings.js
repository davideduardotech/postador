function openSection(sectionId) {
    const sections = ['account', 'security', 'payments'];
...
async function updateAccount() {
    ...
    if (cpf.value.length == 0) {
        alertMessage("error", "Insira o seu CPF")
        return
    }
    ...
    fetch('/app/settings/update-account', {
        method: 'PUT',
        headers: {
            "Content-Type": "application/json",
        },
        body: JSON.stringify({ name: name.value, cpf: cpf.value, email: email.value, phone: phone.value })
    })
    .then(async (response) => {
...
async function updatePassword() {
    ...
    fetch('/app/settings/update-password', {
        method: 'PUT',
        headers: {
            "Content-Type": "application/json",
        },
...
function updateAvatar(input) {
    ...
    fetch('/app/settings/update-avatar', {
        method: 'PUT',
        body: formData
    })
    .then((response) => {
